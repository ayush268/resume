...
%---------------------------------------------------------
  \cventry
    {Under Prof. Tianyin Xu and Dr. Apoorve Mohan} % Role
    {Understanding and Fixing Bottlenecks in QCSC Pipelines} % Event
    {UIUC} % Location
    {Feb 2026 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s)
        \item Vision: Design efficient fault-tolerant QCSC pipelines which provide increased and stable fidelity with least quantum-classical resource usage.
        \item Designed a QEM profiler for understanding the importance and quantum-classical resource usage associated with Quantum Error Mitigation.
        \item Demonstrated multiple dimensions in the QEM pipeline, each of which having significant impact on fidelity and quantum-classical resource usage, with significant potential for optimization.
        \item Desiging an end-to-end workflow manager for quantum workloads over shared HPC-stack having CPUs, GPUs and QPUs.
      \end{cvitems}
    }
...
%---------------------------------------------------------
  \cventry
    {Under Prof. Brighten Godfrey and Prof. Matthew Caesar} % Role
    {My way or the PHY way: Every Packet deserves the right MCS} % Event
    {UIUC} % Location
    {Jan 2025 - July 2025} % Date(s)
    {
      \begin{cvitems} % Description(s)
        \item Vision: End-host devices can significantly improve the QoS of applications without any support from the core network.
        \item Designed and implemented an application-aware rate control mechanism that optimizes per-packet Modulation and Coding Schemes (MCS) based on traffic intent (latency vs throughput).
        \item Reduced tail latency for real-time traffic by eliminating mandatory probing (10-15\% overhead) used in traditional algorithms like Linux Minstrel.
        \item Demonstrated significant performance gains in mixed-workload environments by exposing application semantics to the PHY/MAC layers.
      \end{cvitems}
    }
...
%---------------------------------------------------------
  \cventry
    {Under Prof. Indranil Gupta} % Role
    {Olympus: Semantic query framework for satellite imagery} % Event
    {UIUC} % Location
    {Jan 2025 - Present} % Date(s)
    {
      \begin{cvitems} % Description(s)
        \item Problem: Lack of real-time and semantic querying capabilities for satellite imagery due to high data volume and downlink constraints.
        \item Engineered a real-time semantic query framework for satellite imagery by shifting from image-centric to representation-centric retrieval.
        \item Integrated geospatial foundation models (SatlasNet) to generate onboard embeddings, achieving a 100x reduction in downlink latency.
        \item Designed an efficient parallel search architecture for direct question answering without underlying image access, streamlining global geospatial insights.
      \end{cvitems}
    }
